...
class ShapenetWGANGP():
    def __init__(self):
        ...
        self.model_name = 'categoricalGAN_2'
    ...
    def train(self, epochs, batch_size, sample_interval=50, switch_epoch=None):
        ...
        for epoch in range(epochs):
            ...
            if epoch % sample_interval == 0 or epoch == epochs - 1:
                ...
                self.sample_images_json(epoch, 20)
    ...
    def sample_images_json(self, epoch, num_samples):
        sample_epoch_dir = self.sample_path + "epoch_" + str(epoch) + "/"
        if not os.path.exists(sample_epoch_dir):
            os.makedirs(sample_epoch_dir)

        # feed forward noise to get generated samples
        noise = np.random.normal(0, 1, (num_samples, self.latent_dim))
        gen_structs = self.generator.predict(noise)

        # convert to categorical for rendering
        gen_structs = np.argmax(gen_structs, axis=4)
        gen_structs = self.downsample(gen_structs)

        json_filename = sample_epoch_dir + '/gen_structs_epoch_' + str(epoch) + '.json'

        # write to json for M's renderer
        jset = []
        first = True
        for i, struct in enumerate(gen_structs):
            struct = np.rot90(struct, 1, axes=(1, 2))
            struct = json.dumps(struct.tolist())
            id = "gen_struct_" + str(i + 1)
            jdat = {'structure': struct}
            jdat['id'] = id
            if first:
        ...
        subprocess.call(['node', 'mass_render.js', json_filename, 'GIF', sample_epoch_dir], stdout=subprocess.DEVNULL)
...
if __name__ == '__main__':
    wgan = ShapenetWGANGP()
    wgan.train(epochs=5000, batch_size=32, sample_interval=399)