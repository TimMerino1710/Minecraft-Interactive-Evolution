...
class WGANGP3D():
    ...
    def load_data(self):
        # X = np.load('../ingame house schematics/old format schematic files/combined.npy')
        # X[np.where(X != 0)] = 1
        ...
        #     a, b, c = idx
        #     data[a[i]][b[i]][c[i]] = 1
        # # X = X.reshape([-1, 19, 19, 19, 1])
        # print(data.shape)

        HOUSE_DATASET = []
        HOUSE_DATASET_BIN = []

        house_combined = np.load('../ingame house schematics/old format schematic files/combined.npy')
        ...
        X = HOUSE_DATASET_BIN.reshape([-1, 16, 16, 16, 1])
        return X
...
if __name__ == '__main__':
    wgan = WGANGP3D()
    wgan.train(epochs=1200, batch_size=128, sample_interval=100)